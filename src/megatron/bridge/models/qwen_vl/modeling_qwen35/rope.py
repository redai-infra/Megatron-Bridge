...
class Qwen35VLMultimodalRotaryEmbedding(nn.Module):
    """Multimodal Rotary Embedding for language model.
    only support for qwen35vl
    ...
    def __init__(
        ...
        assert not self.rotary_interleaved, "only support qwen35vl"
...
# Slightly modified from Qwen35VLModel.get_rope_index
def get_rope_index(
    ...
    """Different from the original implementation, Qwen35VL use timestamps rather than absolute time position ids."""
...
def apply_rotary_pos_emb_absolute(
    ...
    In Qwen35-VL, the shape of freqs is (seq_length, bs, 1, 2 * dim) instead of [max_seqlen, 1, 1, 2 * dim]
    """
