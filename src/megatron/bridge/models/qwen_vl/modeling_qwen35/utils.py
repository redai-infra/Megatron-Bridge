...
from megatron.bridge.models.qwen_vl.modeling_qwen35.transformer_config import Qwen35VLTransformerConfig
...
class Qwen35VLVisionPatchEmbed(nn.Module):
    """
    Vision Patch Embed for Qwen35VL vision model.
    """

    def __init__(
        self,
        config: Qwen35VLTransformerConfig,
    ) -> None:
        super().__init__()
...
class Qwen35VLVisionRotaryEmbedding(nn.Module):
    """
    Vision Rotary Embedding for Qwen35VL vision model.
    """
...
class PatchMergerSubmodules:
    """
    Patch Merger Submodules for Qwen35VL vision model.
    """
...
class Qwen35VLVisionPatchMerger(MegatronModule):
    """
    Vision Patch Merger for Qwen35VL vision model.
    """
...
def split_part_by_cp_tp(cp_size, cp_rank, tp_size, tp_rank, split_size):
    """
    Get the split part by CP and TP for Qwen35VL vision model using zigzag pattern.
    """
...
def split_deepstack_embs(
    ...
    """
    Split the deepstack visual embeddings by CP and TP for Qwen35VL vision model.
    NOTE:
...
def find_vision_id_index(
    ...
    """
    Find the vision id index for Qwen35VL vision model.
    """
...
def reorganize_inputs(
    ...
    """
    Reorganize the inputs for Qwen35VL vision model.
    """
...
def split_data_cp_rank(val: torch.Tensor, cp_size: int, seq_dim: int, cp_rank: int = None):
    """
    Split the data by CP rank for Qwen35VL vision model, using zigzag pattern.
    """
    assert cp_size > 1
...
def expand_thw(thw: torch.Tensor) -> torch.Tensor:
    """
    Expand the THW for Qwen35VL vision model.
    """
    assert thw.dim() == 2
...
def collapse_thw(expanded: torch.Tensor) -> torch.Tensor:
    """
    Collapse the THW for Qwen35VL vision model.
    """
...
def qwen2vl_pad_and_split(
    ...
    """
    Split the pixel values and image grid thws for Qwen35VL vision model.
    """
...
def qwen35vl_cp_split(
    ...
    for image_padded in images_padded:
        assert not image_padded, "qwen35vl vit not support sp now, no need to paded"
...
class AllGatherVisionEmbeddings(torch.autograd.Function):
    """
    AllGatherVisionEmbeddings for Qwen35VL vision model.
    """
