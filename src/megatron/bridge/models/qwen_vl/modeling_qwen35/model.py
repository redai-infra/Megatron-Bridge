...
from megatron.bridge.models.qwen_vl.modeling_qwen35.attention import Qwen35VLSelfAttention
from megatron.bridge.models.qwen_vl.modeling_qwen35.rope import get_rope_index
from megatron.bridge.models.qwen_vl.modeling_qwen35.text_model import Qwen35VLGPTModel
from megatron.bridge.models.qwen_vl.modeling_qwen35.transformer_config import (
    Qwen35VLTransformerConfig,
    get_vision_model_config,
)
from megatron.bridge.models.qwen_vl.modeling_qwen35.utils import (
...
from megatron.bridge.models.qwen_vl.modeling_qwen35.vision_model import Qwen35VLVisionModel


class Qwen35VLModel(MegatronModule):
    """Qwen35VL multi-modal model.
    ...
    def __init__(
        ...
        if hasattr(language_transformer_layer_spec, "submodules"):
            language_transformer_layer_spec.submodules.self_attention.module = Qwen35VLSelfAttention
        ...
        if self.pre_process:
            if language_transformer_config.use_hf_vision_model:
                raise ValueError("use_hf_vision_model is not supported for Qwen35VLModel for now")
            ...
            vision_transformer_layer_spec.submodules.self_attention.module = Qwen35VLSelfAttention
            ...
            self.vision_model = Qwen35VLVisionModel(
        ...
        if self.pg_collection.cp.size() > 1:
            assert self.config.calculate_per_token_loss, (
                "Qwen35-VL model only supports context parallelism with calculate_per_token_loss enabled"
            )
    ...
    def set_input_tensor(self, input_tensor) -> None:
        ...
        assert len(input_tensor) == 1, "input_tensor should only be length 1 for Qwen35VL"
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Forward function of the Qwen35VL model.
        ...
        torch.cuda.nvtx.range_push("Qwen35VLModel.forward.pre_process")
            ...
            if vision_grid_thw is not None and vision_grid_thw.shape[0] > 0:
                if cp_size > 1 and self.config.vision_dp_when_cp:
                    if cp_img_num is None:
                        ...
                        vision_data, vision_grid_thw, cp_img_num, images_padded = qwen35vl_cp_split(
                            cp_size,
                            vision_data,
                            vision_grid_thw,
                        )
        ...
        torch.cuda.nvtx.range_push("Qwen35VLModel.forward.language_model")
