...
- A vision encoder (similar to Qwen35-VL)
...
from megatron.bridge.models.qwen_vl.modeling_qwen35.model import Qwen35VLModel
...
class Qwen35VLModelProvider(GPTModelProvider):
    ...
    model architecture with a vision encoder (similar to Qwen35-VL) and a standard
    ...
    #     get_transformer_block_with_experimental_attention_variant_spec
    # )
    transformer_layer_spec: ModuleSpec | Callable[["GPTModelProvider"], ModuleSpec] = get_gpt_decoder_block_spec
    ...
    def provide(self, pre_process=None, post_process=None, vp_stage=None) -> Qwen35VLModel:
...
class Qwen35VLMoEModelProvider(GPTModelProvider):
    ...
    architecture (like Qwen3-Next) with a vision encoder (similar to Qwen35-VL) and
    ...
    def provide(self, pre_process=None, post_process=None, vp_stage=None) -> Qwen35VLModel:
        ...
        challenge is that Qwen35VLModel.__init__ does::
        ...
        get the Qwen35VLSelfAttention override; GDN layers must be left alone.
        ...
        Qwen35VLModel. Because GPTModel → TransformerBlock already accepts
        TransformerBlockSubmodules, we just need to bypass the uniform patch
        in Qwen35VLModel.__init__ by calling MegatronModule.__init__ directly
